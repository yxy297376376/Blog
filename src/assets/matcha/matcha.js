...
function ExSearchCall(item) {
  if (item && item.length) {
    ...
    $.pjax({ url: url, container: "#main", fragment: "#main", timeout: 8000 }); // 发起一次 PJAX 请求
  }
}
...
var linkTarget = function () {
  ...
  $(
    ".post-content a:not([no-linkTarget]), .comment-content a:not([no-linkTarget])"
  ).each(function () {
    if (
      $(this).attr("href").indexOf(host_url) < 0 &&
      $(this).attr("href").indexOf("#") < 0 &&
      !$(this).hasClass("no-linkTarget")
    ) {
      $(this).attr("target", "_blank");
      $(this)
        .prepend('<span class="iconfont external-icon">&#xe888;</span>')
        .addClass("no-linkTarget");
    }
  });
};
//适配 CopyDog 插件
...
$("#nav-drop").click(function () {
  $(".navbar-dropdown").fadeIn().addClass("down");
});
$("#nav-rise").click(function () {
  $(".navbar-dropdown").fadeOut().removeClass("down");
});
...
var doSearch = function () {
  if ($("#input_search").val() == null) {
    ...
    let url =
      ...
      $("#input_search").val() +
      "/";
    $.pjax({ url: url, container: "#main", fragment: "#main", timeout: 8000 });
  }
};
...
$(document)
  .pjax(
    'a[href^="' +
      siteurl +
      '"]:not(a[target="_blank"], a[no-pjax], .cancel-comment-reply a, .comment-reply a)',
    {
      container: "#main",
      fragment: "#main",
      timeout: 8000
  ...
  .on("pjax:send", function () {
    $("body").append(
      '<div class="progress" role="spinner" id="pjax-loading"><div class="progress-bar"></div></div>'
    );
