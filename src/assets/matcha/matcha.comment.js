...
matchaComment.before = function () {
  ...
  $("#comment-form input,#comment-form textarea,#comment-form .submit")
    .attr("disabled", true)
    .css("cursor", "not-allowed");
  $("#comment-form").animate({ opacity: 0.5 }, 300);
};
...
matchaComment.core = function () {
  $("#comment-submit").click(function (e) {
    //判断信息是否完整
    if (
      $('#comment-form [name="text"]').length &&
      !$('#comment-form [name="text"]').val().length > 0
    ) {
    ...
    if (
      $('#comment-form [name="author"]').length &&
      (!$('#comment-form [name="author"]').val().length > 0 ||
        !$('#comment-form [name="mail"]').val().length > 0)
    ) {
        ...
        if ("" === replyTo) {
          ...
          if ($("#" + replyTo).hasClass("comment-parent")) {
            ...
            if ($("#" + replyTo).find(".comment-children").length) {
              $("#" + replyTo + " .comment-children .comment-list")
                .first()
                .prepend(newComment);
